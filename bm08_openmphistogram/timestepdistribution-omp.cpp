...
int main(int argc, char** argv)
{
    ...
    /* In the parallelization, both reading the numbers and computing
       the histogram should be parallelized. */
    #pragma omp parallel default(none) shared(filename,nlines,times)
    {
        std::ifstream filein(filename);
        int thread = omp_get_thread_num();
        int nthreads = omp_get_num_threads();
        int mynlines = (nlines+nthreads-1)/nthreads;
        int mystart = thread*mynlines;
        filein.seekg(9*mystart);
        for (int i = 0; i < mynlines; i++) {
            filein >> times[mystart+i];
            if (filein.bad()) break;
        }
        filein.close();
    }
    double maxt = 0;
    #pragma omp parallel for default(none) shared(times,nlines) reduction(max:maxt)
    ...
    bins.fill(0);
    #pragma omp parallel for default(none) shared(times,nlines,bins,logbase)
    for (int i = 0; i < nlines; i++) {

...
/// read the file (rank zero reads a batch, then scatters, then repeats)
rvector<double> read_data(std::string filename, long long batchsize)
{
    ...
    long long nbatches = ceildiv(nlines,batchsize);     // number of batches
    long long localpointsperbatch = ceildiv(batchsize,size); // number of points per batch per rank
    long long localpoints = localpointsperbatch*nbatches;    // number of points per rank
    rvector<double> localtimes(localpoints);
    rvector<double> times(batchsize);
    ...
    if (0 == rank) {
        ...
        for (int b = 0; b < nbatches; b++) {
            // Read numbers
            times.fill(NA);
            for (int i = 0; i < batchsize && not filein.eof(); i++) 
                filein >> times[i];
            // Distribute
            MPI_Scatter(times.data(), localpointsperbatch, MPI_DOUBLE,
                        localtimes.data() + b*localpointsperbatch, 
                        localpointsperbatch, MPI_DOUBLE, 0,
                        MPI_COMM_WORLD);
        }
    } else {
        for (int b = 0; b < nbatches; b++)
            // Receiving part of the distribution (matching the call in the root process)
            MPI_Scatter(times.data(), localpointsperbatch, MPI_DOUBLE,
                        localtimes.data() + b*localpointsperbatch,
                        localpointsperbatch, MPI_DOUBLE, 0,
                        MPI_COMM_WORLD);
